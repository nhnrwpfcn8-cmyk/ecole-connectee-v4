...
export default function AdminEcoleNotesBulletinsPage({
  ...
  const gradeRows = useMemo(() => {
    return grades
      ...
      .filter(
        (grade) =>
          !studentId || grade.student_id === studentId
      )
      .filter(
        (grade) =>
          !assessmentId ||
  ...
  /* =========================================================
     ...
     Composition
              ↓
     Moy /20 = moyenne(Devoir, Composition)
              ↓
    ...
    return sourceStudents.map((student) => {
      ...
      const subjectsRows = Object.entries(
        ...
        const compositionRow = rows.find(
          (row) =>
            row.assessment_slot === "composition"
        );

        /* -----------------------------------------------------
           DEVOIR
           Moyenne de Devoir 1 et Devoir 2, normalisée sur 20.
        ----------------------------------------------------- */
        ...
        const comp =
          ...
          Number.isFinite(
            ...
            ? (Number(compositionRow.score) /
                (Number(compositionRow.max_score) ||
                  20)) *
              20
            : null;
        ...
        /* -----------------------------------------------------
           ...
           Si devoir + composition :
           moyenne des deux.

           Si seulement devoir :
           devoir.

           Si seulement composition :
           composition.
        ----------------------------------------------------- */
        ...
        /* -----------------------------------------------------
           ...
           Le coefficient est celui de la matière.

           On ne fait PAS :
           coefficient devoir 1
           + coefficient devoir 2
           + coefficient composition.

           On prend le coefficient de la matière.
        ----------------------------------------------------- */
        const coefficientRow = rows.find(
          (row) =>
            Number.isFinite(
      ...
      /* -------------------------------------------------------
         ...
         Le bulletin exemple utilise :

         Somme(Moyenne matière × coefficient)
         -----------------------------------
                    Somme(coefficients)
  ...
  /* =========================================================
     ...
     Le rang est calculé dans la classe sur la moyenne
     générale pondérée par les coefficients.
  ========================================================= */
  const classRankMap = useMemo(() => {
    ...
    activeStudents.forEach((student) => {
      ...
      Object.values(bySubject).forEach(
        (rows) => {
          const devoirRows = rows.filter(
            (row) =>
              row.assessment_slot ===
    ...
    Object.entries(byClass).forEach(
      ([classKey, rows]) => {
        const ranked = rows
          .filter(
            (row) =>
    ...
    activeStudents.forEach((student) => {
      ...
      Object.entries(
        ...
        ([subjectKey, rows]) => {
          ...
          const key = `${classKey}_${subjectKey}`;

          if (!byClassSubject[key]) {
            byClassSubject[key] =
              [];
          }

          byClassSubject[key].push(
            {
              studentId:
                student.id,
              average:
  ...
  ========================================================= */
  async function createSignedBrandingUrl(
    path
  ) {
    if (!path) return "";
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {error && (
        <div
          className="ec-error-card no-print"
          style={{
            marginBottom: 16,
          }}
        >
      ...
      {message && (
        <div
          className="ec-success-card no-print"
          style={{
            marginBottom: 16,
          }}
        >
          ...
          <div className="ec-bulletin-print">
            <div className="ec-bulletin-paper">
              ...
              <table className="ec-bulletin-table">
                ...
                <tbody>
                  {selectedBulletinRow.subjects.map(
                    (subject) => {
                      const subjectRank =
                        subjectRankMap[
                          `${selectedStudent.id}_${selectedStudent.class_id || "unknown"}_${subject.subjectId || "unknown"}`
                        ] || "—";
                  ...
                  <tr className="ec-bulletin-total">
                    ...
                    <td>
                      {formatCoefficient(
                        selectedBulletinRow.totalCoefficient
                      )}
                    </td>

                    <td>
                      {formatAverage(
                        selectedBulletinRow.totalWeighted
                      )}
                    </td>
